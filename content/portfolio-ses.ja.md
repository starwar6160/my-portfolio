...
23年の実務経験を持つシニアソフトウェアエンジニア／アーキテクトです。低レイヤ（C/C++）、ファームウェア、レガシー刷新、Infra、Embedded、高並列バックエンド、障害解析を中心に、複雑なシステムを短期間で把握し、原因不明の問題を再現・切り分け・修正まで持っていくことを強みとしています。
...
- **高並列・高性能バックエンド**: 単機1.6万以上のリアルタイム同時接続を安定して維持するバックエンドの設計・実装。
